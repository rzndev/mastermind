...
namespace MasterMind
{
	...
	public class Board
	{
        ...
        public BFS bfs; // метод поиска в ширину
        public DFS dfs; // метод поиска в глубину
        public EdgeBounds edgeBounds; // стратегия ветвей и границ
        public Grad grad;   // метод градиентного спуска
		...
		{
			// цикл по матрице Grid и размещение
			// окрашенных колышков в заданном месте
			for (int i = 0; i < Grid.GetLength(0); i++)
			{
		...
		public void InitializeBoard()
		{
            ...
            bfs.BuildScoresTable(); // строим таблицу вычисленных очков между двумя комбинациями
            dfs.scores = bfs.scores;
            grad.scores = bfs.scores;
            MM_base.GameTreeNode tree = bfs.BuildTree(); // строим дерево, используемое для решения игры методами в ширину и глубину
            dfs.GameTree = tree;                         // одно и тоже дерево используется для различных методов поиска
            dfs.number_of_nodes = bfs.number_of_nodes;   // соответственно, общее количество вершин в дереве одинаково
		...
		public void AddPeg(int aColor, destination dest)
		{
		  ...
		  if  (CurrentPosition == 4)
			  CurrentPosition = 0;  // позволяем переделать
		}
